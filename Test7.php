        ...
        <div class="container">
        <h3>TEST: Lesson 4 </h3>
        <form class="container" action="Test7.php" method="post" >
        <?php include "connection.php";
        $nRow="SELECT COUNT(*) FROM exam where ID =7";
        $data_Row=$db->query($nRow);
        $number_of_row=$data_Row->fetchColumn();
        echo "This test has <b>".$number_of_row." questions</b>.<br><br>";
        $i=1;
          $myquery="SELECT * FROM exam WHERE ID=7 ";
...
?>
        <input type='submit' class="btn btn-default" name='btnSmTest7' value='Submit' style="margin-left: 500px;" >
        </form>
        </div>
        <?php
        if(isset($_POST['btnSmTest7']))
        {
