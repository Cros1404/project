        ...
        <div class="container">
        <h3>TEST: Lesson 7 </h3>
        <form class="container" action="Test9.php" method="post" >
        <?php include "connection.php";
        $nRow="SELECT COUNT(*) FROM exam where ID =9";
        $data_Row=$db->query($nRow);
        $number_of_row=$data_Row->fetchColumn();
        echo "This test has <b>".$number_of_row." questions</b>.<br><br>";
        $i=1;
          $myquery="SELECT * FROM exam WHERE ID=9 ";
...
?>
        <input type='submit' class="btn btn-default" name='btnSmTest9' value='Submit' style="margin-left: 500px;" >
        </form>
        </div>
        <?php
        if(isset($_POST['btnSmTest9']))
        {
