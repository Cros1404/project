        ...
        <div class="container">
        <h3>TEST: Lesson 5 </h3>
        <form class="container" action="Test8.php" method="post" >
        <?php include "connection.php";
        $nRow="SELECT COUNT(*) FROM exam where ID =8";
        $data_Row=$db->query($nRow);
        $number_of_row=$data_Row->fetchColumn();
        echo "This test has <b>".$number_of_row." questions</b>.<br><br>";
        $i=1;
          $myquery="SELECT * FROM exam WHERE ID=8 ";
...
?>
        <input type='submit' class="btn btn-default" name='btnSmTest8' value='Submit' style="margin-left: 500px;" >
        </form>
        </div>
        <?php
        if(isset($_POST['btnSmTest8']))
        {
